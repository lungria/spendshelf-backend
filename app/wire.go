...
func InitializeApp() (*State, error) {
	wire.Build(
		...
		importer.NewDefaultAccountImporter,
		...
		wire.Bind(new(importer.AccountImporter), new(*importer.DefaultAccountImporter)),
		...
		wire.Bind(new(importer.UserInfoBankAPI), new(*mono.Client)),
